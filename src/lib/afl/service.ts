...
import { matches, players, playerStats, teams } from "$lib/db/schema";
import { desc, eq } from "drizzle-orm";
import type { ScrapedMatch, ScrapedPlayerStat } from "./scraper";
...
export function getOrCreatePlayer(name: string, teamId: string): number {
  ...
  // Race condition fallback — fetch again
  const fallback = db
    .select({ id: players.id })
    .from(players)
    .where(eq(players.name, name))
  ...
  if (!fallback) throw new Error(`Failed to get or create player: ${name}`);
  return fallback.id;
}
