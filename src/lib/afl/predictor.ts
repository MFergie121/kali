...
export interface PredictorInputs {
  ...
  allTeams: Team[];
  yearMatches: YearMatchRow[];
  h2hMatches: H2hMatchRow[];
  basicStatRows: BasicStatRow[];
  advStatRows: AdvStatRow[];
  tipsByGame: Map<number, number>;
}
...
function venueScore(isHome: boolean, venueWinPct: number | null): number {
...
function deriveFactors(
  ...
  const entries: { key: keyof FactorBreakdown; weight: number }[] = [
...
export function computePredictions(inputs: PredictorInputs): PredictionGame[] {
  const {
    roundFixtures,
    allTeams,
    yearMatches,
  ...
  const gamePairs: { fixture: Fixture; homeTeam: Team; awayTeam: Team }[] = [];
  ...
  const teamAvgs = new Map<string, Record<string, number>>();
  ...
  for (const { fixture, homeTeam, awayTeam } of gamePairs) {
    const homeSeason =
      seasonDataMap.get(homeTeam.id) ?? computeSeasonData([], homeTeam.id);
    const awaySeason =
      seasonDataMap.get(awayTeam.id) ?? computeSeasonData([], awayTeam.id);
...
// ─── Data assembly (DB queries) ────────────────────────────────────────────────

export interface AssembleOptions {
  /** Restricts yearMatches to round < this (point-in-time correctness for backfill). */
  upToRound?: number;
  /** Restricts h2h matches to date < this ISO string. */
  beforeDate?: string;
  /** When true, skip Squiggle tips even if rows exist for the round. */
  skipTips?: boolean;
}

export async function assemblePredictorInputs(
  year: number,
  round: number,
  opts: AssembleOptions = {},
): Promise<PredictorInputs> {
  ...
  const yearWhere =
    opts.upToRound !== undefined
      ? and(eq(matches.year, year), lt(matches.round, opts.upToRound))
      : eq(matches.year, year);

  const yearMatches: YearMatchRow[] = await db
    .select({
      id: matches.id,
      round: matches.round,
      year: matches.year,
  ...
  // Compute recent match IDs across all teams (used to scope the team-stats query)
  const allTeamIds = [
    ...new Set(yearMatches.flatMap((m) => [m.homeTeamId, m.awayTeamId])),
  ];
  ...
  const statsMatchFilter =
    recentMatchIds.length > 0
      ? sql`${playerStats.matchId} IN (${sql.join(
          recentMatchIds.map((id) => sql`${id}`),
          sql`, `,
        )})`
      : eq(matches.year, year);
  const advStatsMatchFilter =
    recentMatchIds.length > 0
      ? sql`${playerStatsAdvanced.matchId} IN (${sql.join(
          recentMatchIds.map((id) => sql`${id}`),
          sql`, `,
        )})`
      : eq(matches.year, year);

  const [basicStatRows, advStatRows] = await Promise.all([
    db
      .select({
    ...
    db
      .select({
        teamId: players.currentTeamId,
        contestedPossessions: sql<number>`cast(sum(${playerStatsAdvanced.contestedPossessions}) as int)`,
        turnovers: sql<number>`cast(sum(${playerStatsAdvanced.turnovers}) as int)`,
        games: sql<number>`cast(count(distinct ${playerStatsAdvanced.matchId}) as int)`,
      })
  ...
  // H2H — last 3 years, scoped to team-pairs in the round (optionally before a date)
  const lookups = buildTeamLookups(allTeams);
  const resolvedPairs: { home: string; away: string }[] = [];
  for (const f of roundFixtures) {
    const home = resolveTeam(f.hteam, lookups);
    const away = resolveTeam(f.ateam, lookups);
    if (home && away) resolvedPairs.push({ home: home.id, away: away.id });
  }

  let h2hMatches: H2hMatchRow[] = [];
  if (resolvedPairs.length > 0) {
    const pairConds = resolvedPairs.map((p) =>
      or(
        and(
          eq(matches.homeTeamId, p.home),
          eq(matches.awayTeamId, p.away),
        ),
        and(
          eq(matches.homeTeamId, p.away),
          eq(matches.awayTeamId, p.home),
        ),
      ),
    );

    const baseWhere = and(gte(matches.year, year - 3), or(...pairConds)!);
    const where = opts.beforeDate
      ? and(baseWhere, lt(matches.date, opts.beforeDate))
      : baseWhere;
