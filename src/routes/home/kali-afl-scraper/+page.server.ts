...
import { matches, players, playerStats, apiUsers } from '$lib/db/afl/schema';
import { getMatchesForRoundAndYear, listApiKeysForUser } from '$lib/db/afl/service';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const totalStatRecords = db.select({ c: count() }).from(playerStats).get()?.c ?? 0;
	...
	if (userEmail) {
		const user = db.select().from(apiUsers).where(eq(apiUsers.email, userEmail)).get();
		if (user) {
