...
import { matches, players, playerStats } from "$lib/db/afl/schema";
...
function buildSummary(games: TeamGameRow[], ladderPos: number): TeamSummary {
  ...
  return {
    ...
    avgMargin:
      played > 0 ? fin.reduce((s, g) => s + (g.margin ?? 0), 0) / played : 0,
...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const ladderPos = (id: string) => {
    const i = ladder.findIndex(([t]) => t === id);
    return i >= 0 ? i + 1 : 99;
  };

  // Team games
  const teamMatchRows = yearMatches.filter(
    (r) => r.homeTeamId === selectedTeamId || r.awayTeamId === selectedTeamId,
  );
  ...
  if (matchIds.length > 0) {
    ...
    const top5 = (key: string, rows: typeof statsRows) =>
      [...rows]
        ...
        .slice(0, 5)
        .map((r) => ({ name: r.playerName, val: (r as any)[key] ?? 0 }))
        .filter((r) => r.val > 0);
  ...
  if (compareTeamId && compareTeamId !== selectedTeamId) {
    const cmpRows = yearMatches.filter(
      (r) => r.homeTeamId === compareTeamId || r.awayTeamId === compareTeamId,
    );
    ...
    const h2hRows = await db
      ...
      .where(
        or(
          and(
            eq(matches.homeTeamId, selectedTeamId),
            eq(matches.awayTeamId, compareTeamId),
          ),
          and(
            eq(matches.homeTeamId, compareTeamId),
            eq(matches.awayTeamId, selectedTeamId),
          ),
        ),
      )
