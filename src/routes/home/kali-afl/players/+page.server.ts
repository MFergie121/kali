...
import { getAllPlayerStatsForYear, getAllAdvancedPlayerStatsForYear } from '$lib/db/afl/service';
...
export const load: PageServerLoad = async ({ url }) => {
	...
	const selectedYear = Math.min(
		Math.max(parseInt(url.searchParams.get('year') ?? '') || currentYear, FIRST_YEAR),
		currentYear
	);
	...
	if (rows.length === 0) {
		return { rows, advRows, allYears, selectedYear, leaderboards: [], streaks: [], allPlayers: [], playerGames: {} };
	}

	// Query player stats with full match context for the games tab
	const statsWithContext = await db.select({
	...
	for (const r of statsWithContext) {
		...
		const result: 'W' | 'L' | 'D' | null = margin == null ? null : margin > 0 ? 'W' : margin < 0 ? 'L' : 'D';

		if (!playerGamesMap.has(r.playerId)) playerGamesMap.set(r.playerId, []);
		playerGamesMap.get(r.playerId)!.push({
			round: r.round, matchId: r.matchId, date: r.date,
			opponentName: teamMap.get(oppId)?.name ?? oppId,
			opponentShort: teamMap.get(oppId)?.shortName ?? oppId,
			isHome, teamScore, oppScore, result,
			kicks: r.kicks, handballs: r.handballs, disposals: r.disposals, marks: r.marks,
			goals: r.goals, behinds: r.behinds, tackles: r.tackles, hitouts: r.hitouts,
			goalAssists: r.goalAssists, inside50s: r.inside50s, clearances: r.clearances, clangers: r.clangers,
			rebound50s: r.rebound50s, freesFor: r.freesFor, freesAgainst: r.freesAgainst,
			aflFantasyPts: r.aflFantasyPts, supercoachPts: r.supercoachPts,
		});
	}

	for (const games of playerGamesMap.values()) games.sort((a, b) => a.round - b.round);

	const allPlayers = [...playerGamesMap.entries()].map(([id]) => ({
		id,
		...
		const avgA = gA.length > 0 ? gA.reduce((s, g) => s + g.disposals, 0) / gA.length : 0;
		const avgB = gB.length > 0 ? gB.reduce((s, g) => s + g.disposals, 0) / gB.length : 0;
		return avgB - avgA;
	});
	...
	const leaderboards = statKeys.map(({ key, label }) => {
		...
		const top5 = [...acc.entries()]
			.map(([playerName, { total, count }]) => ({ playerName, avg: count > 0 ? total / count : 0 }))
	...
	// Streaks
	const streaks: { type: string; playerName: string; value: number; label: string }[] = [];

	// Goal streak
	const playerRoundGoals = new Map<string, Map<number, number>>();
	for (const r of statsWithContext) {
		if (!playerRoundGoals.has(r.playerName)) playerRoundGoals.set(r.playerName, new Map());
	...
	for (const [name, roundMap] of playerRoundGoals) {
		...
		for (const r of rounds) {
			if ((roundMap.get(r) ?? 0) > 0) { cur++; if (cur > max) max = cur; } else cur = 0;
		}
		if (max > bestGoalStreak.value) bestGoalStreak = { playerName: name, value: max };
	}
	if (bestGoalStreak.value > 0) {
		streaks.push({ type: 'goal-streak', playerName: bestGoalStreak.playerName, value: bestGoalStreak.value, label: `goals in ${bestGoalStreak.value} consecutive round${bestGoalStreak.value !== 1 ? 's' : ''}` });
	}

	// Disposal machine
	const disposalMachine = new Map<string, number>();
	for (const r of statsWithContext) {
		if (r.disposals >= 30) disposalMachine.set(r.playerName, (disposalMachine.get(r.playerName) ?? 0) + 1);
	}
	const bestDisposalMachine = [...disposalMachine.entries()].sort((a, b) => b[1] - a[1])[0];
	if (bestDisposalMachine) {
		streaks.push({ type: 'disposal-machine', playerName: bestDisposalMachine[0], value: bestDisposalMachine[1], label: `rounds with 30+ disposals` });
	}

	// Hot form (last 3 rounds)
	const allRoundsSorted = [...new Set(statsWithContext.map(r => r.round))].sort((a, b) => a - b);
	...
	if (last3.length > 0) {
		...
		for (const r of statsWithContext) {
			if (last3.includes(r.round)) {
				...
				hotAcc.set(r.playerName, { total: ex.total + r.disposals, count: ex.count + 1 });
			}
		}
		const topHot = [...hotAcc.entries()]
			.map(([name, { total, count }]) => ({ name, avg: count > 0 ? total / count : 0 }))
			.sort((a, b) => b.avg - a.avg)[0];
		if (topHot) streaks.push({ type: 'hot-form', playerName: topHot.name, value: Math.round(topHot.avg * 10) / 10, label: `avg disposals over last ${last3.length} rounds` });
	}
	...
	for (const r of statsWithContext) {
		...
		fantasyAcc.set(r.playerName, { total: ex.total + r.aflFantasyPts, count: ex.count + 1 });
	}
	const topFantasy = [...fantasyAcc.entries()]
		.map(([name, { total, count }]) => ({ name, avg: count > 0 ? total / count : 0 }))
		.sort((a, b) => b.avg - a.avg)[0];
	if (topFantasy) streaks.push({ type: 'fantasy-standout', playerName: topFantasy.name, value: Math.round(topFantasy.avg * 10) / 10, label: `AFL Fantasy avg` });

	const playerGames: Record<number, PlayerGameRow[]> = Object.fromEntries(playerGamesMap);
