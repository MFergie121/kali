...
import { getPlayerStatsPaginated, VALID_PLAYER_STAT_SORT_KEYS } from '$lib/db/afl/service';
...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	if (sortBy && !VALID_PLAYER_STAT_SORT_KEYS.includes(sortBy)) {
		return json({ error: `Bad request: invalid sort_by '${sortBy}'. Valid values: ${VALID_PLAYER_STAT_SORT_KEYS.join(', ')}` }, { status: 400 });
	}
	if (orderParam && orderParam !== 'asc' && orderParam !== 'desc') {
		return json({ error: "Bad request: order must be 'asc' or 'desc'" }, { status: 400 });
	}

	const { data, total } = await getPlayerStatsPaginated({ matchId, playerId, year, round, teamId, sortBy, order, limit, offset });
