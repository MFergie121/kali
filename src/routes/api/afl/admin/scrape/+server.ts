...
export const POST: RequestHandler = async (event) => {
	...
	for (const mid of mids) {
		console.log(`[afl-scraper] scraping mid=${mid} (${matchesScraped + 1}/${mids.length})`);
		const [data, advData] = await Promise.all([
			scrapeMatchStats(mid),
			scrapeMatchAdvancedStats(mid),
		]);
		console.log(`[afl-scraper] persisting mid=${mid} — ${data.homeStats.length} home + ${data.awayStats.length} away players`);
	...
	console.log(`[afl-scraper] complete — year=${year}, round=${round}, matchesScraped=${matchesScraped}`);
